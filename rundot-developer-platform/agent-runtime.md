...
## Park old sessions outside appStorage

Sessions share the game's `appStorage` bucket (128 keys). Compaction reclaims
keys inside one long conversation. Archive is the tool for many sessions.
`deleteSession` leaves a tombstone and does **not** free the cap.

Pass the live `files` object to `createVenusSessionArchive`. The helper hashes
the session id into a valid Files key, uploads a private JSON snapshot, confirms
(with `uploadToken` on overwrite), then purges hot keys. It does not import the
game SDK.

```ts
import { createVenusSessionArchive } from '@series-inc/rundot-agent/venus'

const archive = createVenusSessionArchive(RundotGameAPI.files)

const session = await archive.open(agent, SESSION_ID)

const parked = await archive.parkIfNeeded(agent, {
  close: id => (id === session.id ? session.close() : undefined),
})
```

`parkIfNeeded` parks the oldest live session when package keys or bucket keys
cross the store thresholds. Compact a fat session yourself; this helper parks
many small ones. `open` resumes from `appStorage` when the session is hot, from
Files when a snapshot exists, and creates a new session when neither does.
`park` / `restore` remain available for an explicit id. Persist to Files
**before** purge; the helper does that.

Low-level `inspectStorage` / `exportSession` / `importSession` / `purgeSession`
stay on `Agent` if you want a different blob store.
