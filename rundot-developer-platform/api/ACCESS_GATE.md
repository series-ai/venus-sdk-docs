...
The Access Gate sits in front of sensitive SDK methods (TextGen, ImageGen, AudioGen, SpriteGen, ThreeDGen, and UGC account mutations) and blocks anonymous users from calling them. When a gated method is invoked:
...
**Per-game guest multiplayer.** Platform-owned games opted in by an admin
exempt the four ticket-minting multiplayer methods (`createRoom`,
`joinOrCreateRoom`, `joinRoomByCode`, `matchmakeRoom`) from the anonymous
gate. `getUserRooms` and every other gated API remain gated. The exemption is
enforced server-side at ticket mint; the client-side gate skip only removes
the login prompt. Guest play excludes platform room chat and room listing —
