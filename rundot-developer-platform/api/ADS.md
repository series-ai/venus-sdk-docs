...
{% hint style="info" %}
On desktop and mobile web, `capabilities.ads` is `false`. `isRewardedAdReadyAsync()`
and `isInterstitialAdReadyAsync()` resolve `false`, `showRewardedAdAsync()`
resolves with no reward, and `showInterstitialAd()` resolves without showing
anything. No ad or download prompt appears. Hide ad-dependent controls when
`capabilities.ads` is `false`.
{% endhint %}
