...
> **Migration notice:** `RundotGameAPI.ai` is deprecated. Use `RundotGameAPI.textGen` instead; the interface is identical and both namespaces run the same model-selection cascade. Calls to `RundotGameAPI.ai` still work but log a one-time console warning.
...
console.log(response.model) // The exact model that actually ran.
```
...
`modelClass` selects a stable model class (`'quick'`, `'standard'`, or `'power'`) and lets the platform choose the exact model — this is the recommended pattern. Naming an exact `model` ID instead is an advanced, best-effort alternative; see [Model Selection](#model-selection).

## Model Selection

Pick one of three stable **model classes** and let the platform choose the exact model:

| `modelClass` | Intended for |
| --- | --- |
| `'quick'` | Fast, inexpensive responses — hints, flavor text, simple lookups. This is the default when a request has neither `model` nor `modelClass`. |
| `'standard'` | General-purpose chat and gameplay logic. |
| `'power'` | The hardest reasoning and generation tasks. |

Each class maps to a server-owned default model. The platform can rotate or retire the underlying models without breaking your game, so a class-only call is the recommended, most stable pattern.

### Resolution cascade

Both `model` and `modelClass` are optional. The backend resolves the model that runs as follows:

| `model` | `modelClass` | Result |
| --- | --- | --- |
| valid | absent | exact model |
| valid | valid class | exact model (the class is only the fallback) |
| invalid | `'standard'` | Standard class default |
| absent | `'power'` | Power class default |
| invalid | absent | Quick class default |
| absent | absent | Quick class default |
| any | invalid class string | HTTP 400 |

"Valid" means the exact model ID exists in the platform registry, is enabled and callable, and is permitted by your game's [`allowedModels`](#rundottextgenconfigjson) policy when one is set. An unknown, retired, disabled, or policy-denied exact model does not fail the request — it moves to the class fallback.

An explicit `'standard'` or `'power'` request never silently falls through to Quick: if that class has no eligible model, the call fails — `AI_POLICY_DENIED` when your policy permits nothing in the class, `MODEL_CLASS_UNAVAILABLE` (HTTP 503) when the class has no valid platform default.

{% hint style="warning" %}
**Intentional behavior change:** before this cascade existed, a request naming an unknown or retired exact model failed. It now falls back to the requested class — or to Quick when no class was sent — instead of failing. Every currently successful model-only call keeps selecting the same model.
{% endhint %}

### Resolved-model reporting

The response tells you which model actually ran, after any fallback:

- Non-streaming: `response.model` is the resolved model ID. It can differ from the `model` you sent.
- Streaming: the terminal `done` chunk carries the same value as `done.model` (optional — absent when the backend predates the field).

### Advanced: exact model IDs

{% hint style="info" %}
**Advanced and less stable.** Exact model IDs, `allowedModels` allowlists, and provider-specific options (reasoning effort, sampling parameters, prompt caching, structured output) are advanced controls. The stable contract covers class-based model selection and fallback only: a class default rotation can land on a model that does not support a pinned provider-specific option, which can then fail the request (for example `INVALID_REASONING_EFFORT`). An exact-ID `allowedModels` list can also remove the automatic retirement fallback — if your list contains only retired or invalid models, class fallback finds no permitted model and the call fails with `AI_POLICY_DENIED`.
{% endhint %}

You can still request an exact model directly, optionally with a class as its fallback:

```typescript
const response = await RundotGameAPI.textGen.requestChatCompletionAsync({
  model: 'claude-sonnet-4-6',   // wins while valid
  modelClass: 'standard',       // fallback if the model is invalid or retired
  messages: [{ role: 'user', content: 'Give me a puzzle hint.' }],
})
```

List the current exact model IDs at runtime:

```typescript
const models = await RundotGameAPI.textGen.getAvailableCompletionModels()
console.log('Available models:', models) // string[] of exact model IDs
```
...
> **Tip:** Call `getAvailableCompletionModels()` at runtime to discover the current exact-ID list. Prefer `modelClass` over hard-coding an ID from it.
...
| --- | --- | --- |
| `modelClass` | `'quick' \| 'standard' \| 'power'` | Stable model class. Recommended. Defaults to `'quick'` when both selection fields are absent. See [Model Selection](#model-selection). |
| `model` | `string` | **Advanced, best-effort.** Optional exact model identifier; wins while valid, otherwise falls back to `modelClass` or Quick. List options with `getAvailableCompletionModels()`. |
...
| `reasoningEffort` | `'none' \| 'minimal' \| 'low' \| 'medium' \| 'high'` | Reasoning (thinking) effort for reasoning-capable models. See [Reasoning Effort](#reasoning-effort). |
...
*Advanced, provider-specific option — see the stability note in [Model Selection](#model-selection).*
...
{% hint style="warning" %}
**The accepted values are per-model, and per model *version*.** There is no single set.
`gpt-5` rejects `'none'` and requires `'minimal'`; `gpt-5.2` rejects `'minimal'` and requires
`'none'`. `gpt-6-astra`, `grok-4.6`, `glm-5.3-flash` and `gemini-2.5-pro` reject `'none'`
outright — their reasoning cannot be switched off at all. If you do not want to track this,
use a `modelClass` and leave `reasoningEffort` unset.
{% endhint %}

| Provider | Support |
| --- | --- |
| OpenAI (`gpt-5` family) | Passed through natively as `reasoning_effort`. Accepted values differ per model version — see the warning above. |
| Gemini | Translated to a thinking level (Gemini 3.x) or a thinking-token budget (Gemini 2.5). Gemini 3.x cannot disable thinking, so `'none'` resolves to the lowest level that family accepts. Older Gemini models (2.0 and earlier) reject the field with HTTP 400. |
| Anthropic (`claude-*`) | Forwarded. **Whether the model honours it varies**: `claude-haiku-4-5` does, `claude-sonnet-5` accepted every level and produced ~0 reasoning tokens regardless (measured 2026-09-20). Check `usage.reasoning_tokens` rather than assuming a 200 means the setting applied. |
| DeepSeek | Forwarded. `deepseek-r1` rejects `'none'` — its reasoning is mandatory. |
...
- **Setting an effort can turn reasoning ON.** `claude-haiku-4-5` ships with thinking off and produces 0 reasoning tokens by default; asking for `'low'` produced 516–897 reasoning tokens across samples. On a model that does not reason by default, `'low'` means *some* where there was *none*.
- **Reasoning tokens come out of your `maxTokens` budget.** If the model spends the whole budget thinking you get `finish_reason: 'length'` with empty content — and you are still billed for the reasoning. Lower the effort rather than raising `maxTokens`.
...
| `id`, `created`, `object`, `ullm_id` | `string` / `number` | Call metadata. |
| `model` | `string` | The resolved model ID that actually ran, after any class fallback — can differ from the requested `model`. |
...
TextGen is the **low-level transport**: you send messages, you get a completion back, and everything around that — running the tool the model asked for, persisting the conversation, recovering after a refresh, retrying a dropped stream, summarising a long history, pausing for the player's approval — is yours to build.

If you want that machinery instead of writing it, install the opt-in agent package:
...
```ts
import { createAgent } from '@series-inc/rundot-agent'
import { createTextGenTransport, createVenusSessionStore } from '@series-inc/rundot-agent/venus'

const agent = createAgent({
  model: createTextGenTransport(rundot.textGen, { mode: 'open' }),
  models: ['claude-sonnet-4-6'],
  tools: { /* your tools */ },
  store: createVenusSessionStore(rundot.appStorage, {
    namespace: 'my-game/assistant',
    concurrency: 'compare_and_swap',
  }),
})

const sessions = await agent.listSessions()
const session = sessions.some(item => item.id === 'assistant')
  ? await agent.openSession('assistant')
  : await agent.createSession({ id: 'assistant' })
const result = await session.send({ text: 'What should I build next?' })
```

Three things to know before you reach for it:

- **It is a separate package.** Importing TextGen does not bundle the agent runtime, and the agent package does not import the SDK at runtime — it adapts `textGen` and `appStorage` structurally, so there is no version to keep in sync.
- **Open mode is required.** A model-authored tool loop cannot run against a templated game; the transport fails at construction with `TEMPLATED_MODE_UNSUPPORTED`. See [`mode`: open vs templated](#mode-open-vs-templated).
- **Use compare-and-swap storage.** It prevents two tabs, devices, or workers from owning the same session at the same time. Use `singleWriter: true` only when your application controls one writer for the full storage lifetime.
- **appStorage is the hot tier.** Sessions share the game's 128-key bucket. `createVenusSessionArchive(rundot.files)` parks old journals in the Files API when the bucket is under key pressure. See [Park old sessions outside appStorage](../agent-runtime.md).
- **TextGen is not going anywhere.** If you just want a completion, keep calling `requestChatCompletionAsync` directly. The agent package is additive.

Read [Agent Runtime (BETA)](../agent-runtime.md) for complete setup, tools,
durable questions, approvals, reload recovery, React events, and multiple-tab
conflict handling. The npm package also contains the API reference and release
information: <https://www.npmjs.com/package/@series-inc/rundot-agent>.
...
| `done` | `finishReason: string`, `usage`, `model?: string` | Terminal chunk. Carries the stop reason, aggregate token totals (same shape as the non-streaming `usage`), and `model` — the resolved model ID that ran (absent on backends that predate the field). Always last. |

Errors are out of band: the iterable throws.

On the playground HTTP path (`HttpAiApi`), the throw is a `RundotApiError`:

| Field | When |
| --- | --- |
| `code` | JSON/SSE machine code when the server sent one (`CREDITS_EXHAUSTED`, …). The HTTP `error` field is copied as-is and is sometimes prose (`Too Many Requests`). A stream that ends without `done` or `error`, or a 200 with no body, uses `NETWORK_ERROR`. |
| `status` | HTTP status when the failure had one (non-OK POST, or SSE `statusCode`). `0` when there was no HTTP status. |

The hosted RPC path still throws a plain `Error` except for credits exhaustion. `@series-inc/rundot-agent` classifies retries from `code` and `status`. It does not parse the message string. An unrecognized `code` such as `STREAM_FAILED` does not override `status`.
...
| `allowedModels` | `string[]` | **Advanced.** Allowlist of permitted exact model identifiers. When set, selection (exact and class fallback) is confined to listed models; a list of only retired/invalid models removes the automatic retirement fallback and calls fail with `AI_POLICY_DENIED`. Omit to allow any. |
...
For `textGen`, `allowedModels` entries are exact model IDs matched during model selection — against a requested `model` and against class-fallback candidates alike (see [Model Selection](#model-selection)).
...
## Instant Decisions (`decide`)

{% hint style="warning" %}
**Alpha, gated.** `decide` is behind the `alpha_ai_decide` Statsig gate. A call from an
ungated account returns `403 ALPHA_FEATURE_GATED`. Ask before you build on it.
{% endhint %}

`decide` answers typed questions about a piece of state. It does not generate text: you
supply the question and the options, and it returns a probability or a choice. That makes
it useful for judgements a game needs quickly and repeatedly — classifying a player's
free-text input, rating a screenshot, or deciding whether something is safe to show.

```typescript
import RundotGameAPI from '@series-inc/rundot-game-sdk/api'

const { answers, usage } = await RundotGameAPI.textGen.decide({
  state: { playerMessage: 'i want to trade my sword for the lantern' },
  questions: {
    intent: {
      type: 'choice',
      instructions: 'What does the player want to do?',
      criteria: { trade: 'exchange items', attack: 'start combat', talk: 'just chatting' },
    },
    hostile: { type: 'noul', instructions: 'The message is hostile or abusive.' },
  },
})

answers.intent.choice // 'trade' | 'attack' | 'talk' — narrowed to your own option keys
answers.hostile.noul  // 0..1 probability
usage.creditsDebited  // pre-markup estimate, not the ledger debit — see CREDITS.md
```

The answer type is inferred from the questions you pass, so `answers.intent.choice` is a
union of your literal option keys rather than a bare `string`.

### Question types

| Type | Ask it | You get back |
|---|---|---|
| `noul` | Whether a statement is true | `{ noul }` — a probability in `[0..1]`. **No `confidence` field.** |
| `choice` | Which option fits | `{ choice, confidence, probabilities? }` — `probabilities` is keyed by your option names |
| `score` | Where it sits on a scale | `{ score, confidence, legend?, probabilities? }` |

A `score` is **continuous**, not an integer index: asking for
`criteria: ['trivial', 'minor', 'moderate', 'major', 'critical']` can return `1.95`, meaning
between `minor` and `moderate`. The optional `legend` maps index strings to your labels, and
`probabilities` is keyed by those same index strings.

### Images

Put a canvas or an image data URL in the state and it is described before the question is
answered:

```typescript
await RundotGameAPI.textGen.decide({
  state: { canvas: myCanvas, level: 3 },
  questions: {
    stuck: { type: 'noul', instructions: 'The player appears stuck on this level.' },
  },
})
```

A `HTMLCanvasElement` is serialized for you. An image costs two credits reserved instead of
one, and the image itself never reaches the judging model — only a description of it does.
...
| Options per `choice` | 1–255 |
| State nesting depth | 16 |
| State + questions, as text | 24 KiB |
| Inline image, decoded | 5 MB |

The text limit is measured **after** images are replaced with a marker, so a large canvas
does not count against it. These are byte and count limits: the underlying provider also
documents a token cap, which the platform does not enforce, so a request near the text
limit with unusually dense content can still be rejected upstream.

### Failures

| Status | Error | Meaning |
|---|---|---|
| 400 | `MISSING_STATE`, `MISSING_QUESTIONS`, `TOO_MANY_QUESTIONS`, `INVALID_QUESTION`, `STATE_TOO_LARGE`, `STATE_TOO_DEEP`, `IMAGE_TOO_LARGE` | The request broke a limit above. Nothing was charged. |
| 400 | `INVALID_GAME` | The `gameId` is not one you can call for. |
| 402 | — | Out of credits. Raised as `CreditsExhaustedError`. |
| 403 | `ALPHA_FEATURE_GATED` | The alpha gate is closed for this account. |
| 502 | `DECIDE_UPSTREAM_INVALID` | The provider answered with something unusable. |
| 504 | `DECIDE_TIMEOUT` | The provider did not answer in time. |
| 500 | `AI_DECIDE_FAILED` | Anything else. |

A judgement is never retried automatically: a retry costs a second call and can answer a
question your game has already acted on.
