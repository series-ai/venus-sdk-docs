...
|--------|---------|-------------|
| `textGen.requestChatCompletionAsync(request)` | `Promise<{ message }>` | Generate text completion |
...
- Both `textGen.requestChatCompletionAsync` and `imageGen.generate` are subject to per-creator rate-limit tiers — see [Rate Limits](RATE_LIMITS.md).
