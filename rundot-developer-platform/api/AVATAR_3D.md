...
Open the native RUN.game 3D avatar editor, load and save player avatars, and pull the avatar asset manifest so your game can render the same characters players build platform-wide.
...
Opens the native RUN.game 3D avatar editor as a pushed app. The editor takes over the screen, and the promise resolves once the player closes it. When the player saves, the host persists the new avatar and the result reports the change.
...
The avatar asset manifest (`assets.json`) lives on the RUN.game CDN and lists every avatar mesh available, grouped by category (head, outfit, hat, hair, face accessory, animation). Use it to render avatars yourself or to preload meshes before a match.
