...
|---|---|
| **T1** (trusted creator) | Nothing — auto-granted server-side |
| **First game on this origin** (device permission not yet granted) | Browser/OS permission prompt only — serves as the single opt-in; platform consent is silently persisted after the browser grants access |
| **Second game on same origin** (device permission already granted) | Platform consent dialog — the browser won't prompt again, so the user needs a gate for this new game |

This means users see exactly **one** prompt per consent event, never two.
...
Request capture consent. The SDK applies the single opt-in logic: T1 apps short-circuit to `granted`; if the browser hasn't granted device permission yet, returns `granted` (the browser will prompt during recording); otherwise shows the platform consent modal.
