...
- **Local testing**: recording is unavailable during local Playground testing; verify clips in the RUN.world app
- **Upload size cap**: limited by `files.getQuota().maxFileBytes` (typically 200 MB); tune `maxDurationMs`, `fps`, and `videoBitsPerSecond` accordingly
