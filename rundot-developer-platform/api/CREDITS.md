...
> **Note:** Creator credits are **not** the same as RUN Bits. RUN Bits are the in-game hard currency, handled by the [Purchases API](PURCHASES.md) (`RundotGameAPI.iap`). Creator credits are the AI credits that power generation in player-billed games. This API (`RundotGameAPI.credits`) only deals with Creator credits.
...
`onBalanceChanged` fires whenever the player's spendable balance changes — both when they buy credits at the paywall and when they spend credits on AI generation. It returns an unsubscribe function.

Rapid changes are coalesced, so a burst of generations finishing together may deliver a single update; that update always reflects the final balance.

It does not currently fire for free-daily resets, for top-ups made outside the game, or for streaming AI completions.
...
`sprite-character-animate`, `tts`, `text`, and `upscale` requests.

`upscale` prices the Topaz upscale exactly as billing brackets it: pass the
source `contentType` — `'image/*'` quotes the flat image rate; `'video/*'`
quotes per second by `resolution` (`'720p'` or `'1080p'`) × `durationSeconds`.
Passing `outputMp` (output megapixels) selects the tiered advanced-upscale
pricing instead.

`quantity` multiplies the quote for a batch of identical generations (default
`1`) — a batch of N scene clips is `{ kind: 'video', …, quantity: N }`. Text
ranges scale bound-for-bound; the percent bounds are unchanged. Pass the
per-unit options (model, mode, duration, size) exactly as generation will send
them, or the quote prices a different product than the one you dispatch.
...
Costs vary by surface. One worth knowing because it is cheap and easy to call in a loop:

| Call | Base credits |
|---|---|
| `textGen.decide`, text only | 1 reserved, 1 settled |
| `textGen.decide`, state carrying an image | 2 reserved, settled at the real cost (1 base in practice) |

These are **base** amounts. A markup multiplier is applied to both the reservation and the
settlement before they reach the ledger, so the credits actually deducted are higher —
with the default multiplier of 2, a text call reserves and settles 2. Settlement also
depends on token usage, and `usage.creditsDebited` on the response is the **pre-markup**
estimate, not the ledger debit. Read the balance if you need the real figure.

What is stable is the shape: `decide` has a flat base minimum of one credit per call, so a
game asking one question per frame pays per call rather than per token. Batch independent
questions into a single `decide` call — it answers them all in one pass for the same
minimum. See [AI API → Instant Decisions](AI.md#instant-decisions-decide).
