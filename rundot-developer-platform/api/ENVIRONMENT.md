...
//     purchases: true   // any in-app purchase flow can be initiated here
//   }
// }
...
| `capabilities.purchases` | `boolean` | Any in-app purchase flow can be initiated on this platform — gates every purchase-initiation surface (store, paywall, shop, subscription) |
...
> that). `capabilities.purchases` covers **every** purchase-initiation surface
> the host exposes (store, paywall, shop, subscriptions); when it is `false`,
> hide all purchase-initiating UI, not just one button. Branch your UI on these
> flags at first render. For backward compatibility, a host that predates this
> field omits it and each capability defaults to `true`, so existing behavior is
> preserved on web and mobile.
