...
2. **`confirmUpload(key)`** — called after your `fetch()` PUT succeeds. The server verifies the object, probes media metadata, transitions to `active`, and updates the creator's storage counter. For public uploads (`visibility: 'public'`), content moderation runs before activation. This runs asynchronously via the job system — the SDK awaits completion automatically.
...
Finalize an upload. Extracts media metadata and transitions the file to `active`. For public uploads (`visibility: 'public'`), content moderation runs before activation — rejected content throws `FILES_CONTENT_REJECTED`. Returns the finalized file entry.
...
Toggle file visibility. Only the file owner can change visibility. When changing from `private` to `public`, content moderation runs before the transition completes — if rejected, the file stays private and `FILES_CONTENT_REJECTED` is thrown.
...
Poll for completed async jobs (`confirmUpload`, `upscale`, `transform`) that finished since last poll. Useful for recovering results after the player leaves and returns.
...
File API calls are rate-limited per creator tier. **Reads and writes have separate budgets** — loading files won't prevent uploads, and vice versa.
