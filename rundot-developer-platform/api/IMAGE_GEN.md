...
| `gemini-3-pro-image-preview` | Nano Banana Pro | Standard | 150 credits ($0.150)* | Studio-quality 4K visuals, complex layouts, precise text rendering (*4K: 300 credits / $0.300). |
...
Not every model in this table is callable by every game: `gpt-image-2`,
`gpt-image-2.5-flare`, and `gpt-image-2.5-sunburst` require a higher creator tier (`power`),
and the server rejects a lower-tier game's request at call time. Use
[`listModels()`](#discovering-available-models) to ask rather than to find out from a failure.
...
Inside a game, call it with no argument — the host scopes it to the running
game. The optional `gameId` argument scopes to a different game's owner tier and
is intended for tooling, not for in-game use.
...
| `imageGen.listModels(gameId?)` | `Promise<readonly string[]>` | Models this caller may invoke (unbilled) |
...
outside this SDK surface — reach them through `rundot`, not
`RundotGameAPI.imageGen`. They are not covered by the stability guarantees of
