...
| `gemini-3.1-flash-lite-image` | Nano Banana 2 Lite | Cheapest Gemini option; high-volume, lower-stakes generation. |
...
`gpt-image-2` rates under a model you did not choose**. Pass `gpt-image-2`
explicitly.
{% endhint %}

Not every model in this table is callable by every game: `gpt-image-2` requires
a higher creator tier, and the server rejects a lower-tier game's request at
call time. Use [`listModels()`](#discovering-available-models) to ask rather
than to find out from a failure.
