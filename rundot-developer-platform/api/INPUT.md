...
and `RundotGameAPI.input` reports the same button names, axis signs, and analog
...
if (RundotGameAPI.input.isSupported()) {
  const onConnected = RundotGameAPI.input.onConnected((event) => {
    console.log(`controller ${event.index} connected: ${event.id}`)
  })

  const onDisconnected = RundotGameAPI.input.onDisconnected((event) => {
    console.log(`controller ${event.index} disconnected`)
  })

  // Poll inside your own requestAnimationFrame loop:
  function tick(): void {
    for (const pad of RundotGameAPI.input.getGamepads()) {
...
```typescript
const sub = RundotGameAPI.input.onConnected((event) => {
...
`input.__debug.getLatencyStats()` returns `{ p50, p95, max, sampleCount }` in ms
...
logs a structured `input.latency` event to its observability pipeline.

```typescript
const { p95, sampleCount } = RundotGameAPI.input.__debug.getLatencyStats()
```
