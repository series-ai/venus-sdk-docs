...
> `game.config.prod.json` is a separate file for local CLI metadata (`gameId`, `relativePathToDistFolder`). Leaderboard config does not go there.
...
* Configure score bounds, durations, and anti-cheat settings in your `config.json` (server config).
