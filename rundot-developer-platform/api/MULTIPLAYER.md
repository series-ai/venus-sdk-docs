...
| `onTick()` | Server clock tick (when `tickInterval` is configured on a persistent room). See [Advanced Multiplayer](ADVANCED-MULTIPLAYER.md#the-server-owned-clock-ontick). |
...
onCreate() {
  ...
  this.clock.setInterval('tick', () => this.handleTimerTick(), 1000)
}
...
onRestore(snapshot: Record<string, unknown>) {
  ...
  this.clock.setInterval('turnTimer', () => this.handleTimerTick(), 1000)
