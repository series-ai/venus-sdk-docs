...
## Authentication and guest play

Realtime multiplayer requires a signed-in user: `createRoom`,
`joinOrCreateRoom`, `joinRoomByCode`, `matchmakeRoom`, and `getUserRooms`
reject anonymous users with `AccessDeniedError` (see [ACCESS_GATE.md](ACCESS_GATE.md)). With
auto-prompt enabled, the SDK shows the login sheet and retries after a
successful sign-in, so an anonymous player can convert mid-session.

Platform-owned games can be opted in to **guest play** by a platform admin
(`multiplayerPolicy.allowAnonymous` on the game record). In an opted-in game,
anonymous users can create, join, and matchmake without signing in. Guest
sessions differ from signed-in sessions:

- Guests appear in rosters as `Guest-NNNN` (server-assigned; not persistent).
- Room chat (`room.send` is unaffected — this is the platform `chat:*`
  channel) is unavailable to guests: sends and history are rejected, and
  chat messages are not delivered to guest members.
- `getUserRooms` still requires sign-in.
- The server re-checks the game policy when minting the join ticket; the
  opt-in cannot be spoofed client-side.
...
> **One config for deterministic rooms.** A fileless entry in
> `rundot/realtime.config.json` is used by the local Vite authority, copied to
> `dist/rooms.config.json` during build, and derived into hosted-playground
> server config. Do not add `rundot/rooms.config.json`, a `GameRoom` subclass,
> or a server bundle for a deterministic room.
  ...
  "rooms": [
    {
      "type": "deterministic",
      "deterministic": true,
      "config": { "maxPlayers": 2, "tickRateHz": 60 }
    }
  ]
}
```

This configuration registers the built-in deterministic authority. It does not
add runtime metadata to a room creation request. Use `createSyncplayRoom()` or
`quickMatchSyncplayRoom()` from `@series-inc/rundot-syncplay/browser` to create
the room. A direct `createRoom()` or `joinOrCreateRoom()` request must include
`createOptions.metadata.syncplayRuntime` with the runtime identity, canonical
session configuration bytes, and session configuration digest.

The SyncPlay creation helpers use the room type `deterministic`. Use that type
in this configuration when you use those helpers.

> Keep `rundotMultiplayerPlugin()` in **both** serve and build config. On serve it
> starts the local deterministic authority with fake local profiles; on build it
> copies the fileless room config to `dist/rooms.config.json` and emits **no**
> `dist/server-bundle.js` when every room is deterministic. `rundotSyncplayPlugin`
> is a separate plugin with a different job — it statically certifies your
> simulation sources and does not run or emit room config.
...
| `deterministic` | `boolean \| object` | `false` | When true, the room runs on the built-in deterministic input authority (Syncplay). No `file`/`GameRoom` and no server bundle are required or emitted; only inputs cross the wire. Use the object form to set the options in the table below. |
| `file` | `string` | *required for server-authoritative rooms* | Path to the file exporting the `GameRoom` subclass, relative to project root. Omit it for `deterministic: true` rooms. |
...
### Deterministic room options

Use the object form — `"deterministic": { ... }` — to configure the built-in
authority. Every field is optional.

| Field | Type | Default | Description |
|---|---|---|---|
| `protocol` | `string` | `"rdm-local-authority/v2"` | Session protocol version. Only the current value is accepted. |
| `simulationHooks` | `object` | - | Server-side bootstrap and settlement hooks (`bootstrap`, `settleRecipeId`, `settlementTarget`). |
| `secrets` | `object` | - | Secret systems for hidden choices, draws, decks, bags, and roles. See [SYNCPLAY-SECRETS](SYNCPLAY-SECRETS.md). Supports 1–16 simulation slots. |
| `reconfigurePolicy` | `"creator" \| "any" \| "none"` | `"none"` | Who may swap the session config over the wire. See [SYNCPLAY-EDITOR](SYNCPLAY-EDITOR.md). |
| `commandsPerSecond` | `integer` | `60` | Per-slot command ingress ceiling per second. |
| `runtime` | `object` | - | The runtime this build simulates, stamped by `rundotMultiplayerPlugin({ syncplayRuntime })`. Do not hand-write it. When present, the platform binds every room to these rules and rejects a join ticket that asks for different ones. |

A room that sets `secrets` must also set `config.playerCount`.

#### Pinning your runtime

Without a pin, a room's rules come from whatever the joining client sends. With
one, the platform accepts only rooms matching your published build.

Mint the pin from the runtime config your simulation module already exports, and
hand it to the plugin:

```ts
// vite.config.ts
import { createSyncplayRuntimePin } from '@series-inc/rundot-syncplay/node'

import { myGameRuntimeConfig } from './src/sim/runtime'

export default defineConfig({
  plugins: [
    rundotMultiplayerPlugin({
      syncplayRuntime: createSyncplayRuntimePin(myGameRuntimeConfig),
    }),
  ],
})
```

`createSyncplayRuntimePin()` throws while your vite config is evaluated if the
identity is malformed, the session config is empty or over 64 KiB, or the digest
does not match the bytes — so a wrong pin fails the build instead of shipping.

One build declares one simulation runtime: every deterministic room in the
config receives the same pin. A game needing two different simulations needs two
builds.
...
| `playerCount` | `number` | `maxPlayers` | Simulation slot count for a deterministic room. A join ticket may lower `maxPlayers` for one room, but never this. **Required** when the room sets `deterministic.secrets`. |
