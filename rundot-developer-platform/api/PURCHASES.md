...
Let players spend RUN Bits to buy digital goods and entitlements in your game. The SDK handles platform billing flows: your game just requests balances and initiates transactions.

## What Are RUN Bits?

RUN Bits are the platform's hard currency. Players acquire RUN Bits through the platform store, then spend them inside games for digital goods, power-ups, cosmetics, or other entitlements.
...
> **Note**: "In-App Purchases" refers to FIAT transactions on the platform or in-game. This API specifically handles **RUN Bits purchases**: transactions where users spend RUN Bits inside your game.
...
// Check player's RUN Bits balance
const balance = await RundotGameAPI.iap.getHardCurrencyBalance()
console.log(`Player has ${balance} RUN Bits`)

// Spend RUN Bits on an item
...
```typescript
// Launch the native RUN Bits store. The result tells you whether a purchase
...
<table><thead><tr><th width="276.2265625">Method</th><th>Returns</th><th>Description</th></tr></thead><tbody><tr><td><code>getHardCurrencyBalance()</code></td><td><code>Promise&#x3C;number></code></td><td>Get player's current RUN Bits balance</td></tr><tr><td><code>spendCurrency(productId, amount, options?)</code></td><td><code>Promise&#x3C;{ success: boolean; error?: string }></code></td><td>Spend RUN Bits on an item. Accepts an optional <code>options</code> object (see below). On <code>success: false</code>, <code>error</code> carries the reason (<code>'USER_CANCELLED'</code> when the player declines).</td></tr><tr><td><code>openStore()</code></td><td><code>Promise&#x3C;{ purchased: boolean; newBalance: number }></code></td><td>Open the native RUN Bits store. Read <code>purchased</code> and the refreshed <code>newBalance</code> after it closes instead of refetching the balance.</td></tr><tr><td><code>getCurrencyIcon(options?)</code></td><td><code>Promise&#x3C;{ base64Data: string }></code></td><td>Get the currency icon as raw base64. Wrap it in a data URI (<code>'data:image/png;base64,' + icon.base64Data</code>) to use as an <code>&#x3C;img></code> src. Optionally pass <code>{ variant, size }</code> to pick the treatment (<code>gradient</code> or the tintable <code>mono</code>) and artwork tier — see the full reference below.</td></tr><tr><td><p><code>isUserSubscribed(</code></p><p><code>tier)</code></p></td><td><code>Promise&#x3C;boolean></code></td><td>Check if the user has a certain subscription tier. Will also return true if the user has a higher subscription tier.</td></tr><tr><td><p><code>getSubscriptions(</code></p><p><code>tier?)</code></p></td><td><code>Promise&#x3C;RunSubscriptionsResponse></code></td><td>Get a list of subscriptions available by tier. Use these to show a paywall to the user. You can optionally pass a tier to this method to get subscriptions for that tier only.</td></tr><tr><td><code>purchaseSubscription(tier, interval)</code></td><td><code>Promise&#x3C;PurchaseSubscriptionResponse></code></td><td>Trigger a checkout flow for a given subscription (identified by its tier and interval). Rejects with a <code>RundotApiError</code> (<code>UNSUPPORTED_SUBSCRIPTION_INTERVAL</code>) if the tier does not offer that interval — LITE is weekly-only.</td></tr><tr><td><code>hasUserMadePurchase()</code></td><td><code>Promise&#x3C;boolean></code></td><td>Check if the user has ever made a purchase on RUN</td></tr><tr><td><code>listDirectPurchaseSkus()</code></td><td><code>Promise&#x3C;DirectPurchaseSku[]></code></td><td>List your game's active real-money items as a per-store pricing view derived from your tagged shop config. Always scoped to the running game — there is no way to read another game's items. Read-only — author these in <code>shop.config.json</code> and buy them with <code>shop.purchase</code>. See <a href="#direct-purchase-catalog">Direct Purchase Catalog</a>.</td></tr></tbody></table>
...
Opens the native RUN Bits store and resolves once it closes. The result lets you react to a purchase without a separate balance fetch. `OpenStoreResult` is a named, importable type from the package root, `@series-inc/rundot-game-sdk` (not the `/api` subpath).

<table><thead><tr><th>Field</th><th>Type</th><th>Description</th></tr></thead><tbody><tr><td><code>purchased</code></td><td><code>boolean</code></td><td>Whether a purchase was completed during the store session.</td></tr><tr><td><code>newBalance</code></td><td><code>number</code></td><td>The updated RUN Bits balance after the store closed (saves a refetch).</td></tr></tbody></table>

### `getCurrencyIcon(options?): Promise<{ base64Data: string }>`

Returns the RUN Bits icon as base64. Use it in an image source as shown below.
...
Defaults to `gradient` at `md` size. When testing locally, variant and size options are ignored.

<table><thead><tr><th>Option</th><th>Type</th><th>Default</th><th>Description</th></tr></thead><tbody><tr><td><code>variant</code></td><td><code>'gradient' | 'mono'</code></td><td><code>'gradient'</code></td><td>Which treatment of the mark to serve.</td></tr><tr><td><code>size</code></td><td><code>'xs' | 'sm' | 'md' | 'detail'</code></td><td><code>'md'</code></td><td>Artwork tier — pick the one nearest your rendered size.</td></tr></tbody></table>

**Choosing a variant.** `gradient` is the filled brand chip and carries the most brand recognition. `mono` is a line mark drawn in pure white with the shape in the alpha channel, so it takes a tint cleanly — reach for it when the gradient fights your palette, and tint it rather than hand-recolouring the PNG. Which one serves your game is your call.

The two are different artwork, not one silhouette recoloured, so they don't share a bounding box — don't assume identical optical weight when swapping between them in a fixed-size slot.

**Choosing a size.** Each tier is drawn for its display range rather than downsampled from one master, and its canvas is 4x the top of that range, so you only ever downscale.

| Tier | Draw it at | Canvas |
|---|---|---|
| `xs` | ≤16px | 64² |
| `sm` | 16–24px | 96² |
| `md` | 24–48px | 192² |
...
`detail` in `gradient` is ~80KB before base64 encoding — request it only when you actually draw the mark large.

```typescript
// A tintable mark for a HUD that runs on your own palette.
const icon = await RundotGameAPI.iap.getCurrencyIcon({ variant: 'mono', size: 'sm' })
```

Older versions of the RUN app may ignore these options and return their default icon.

## Direct Purchase Catalog

Direct purchases are **real-money** products (FIAT, billed through the platform store — App Store, Google Play, Steam, or web checkout), unlike `spendCurrency`, which spends RUN Bits. The platform handles billing, fulfillment, and refunds server-side.

{% hint style="warning" %}
**This catalog is a derived, read-only view. You do not author it here.**

A real-money item is a normal shop item whose price pins a platform price tier:

```json
{
  "itemId": "sword",
  "price": { "type": "direct_purchase", "value": "tier_499" },
  "entitlements": [{ "entitlementId": "sword", "quantity": 1, "consumable": false }]
}
```

Put that in `rundot/shop.config.json`, upload the server config, and point a tag
at it. This catalog is then generated from that tagged config. See
[SHOP.md](SHOP.md#selling-for-real-money).

**Buy with `RundotGameAPI.shop.purchase(itemId, idempotencyKey)`**, not with an
IAP method. There is no `iap.purchaseDirectSku`.
{% endhint %}

### Key Architecture: File-Only Configuration

- **No Store Console or Dashboard Setup**: Developers do **not** configure products or SKUs on platforms like Google Play or the App Store. File configuration in your project is all that is necessary. RUN operates a universal platform price ladder where all store products are pre-provisioned and shared across all games.
- **How SKUs are Provisioned**: SKUs are never created individually per game. Instead, your game pins an item in `rundot/shop.config.json` to a platform `tierId` (e.g., `tier_099`, `tier_499`). When you deploy or upload your server config (`rundot game upload-server-config`), the platform automatically resolves the store products, handles checkout, and fulfills entitlements server-side.
- **Supported Storefronts (Web, iOS, Android, Steam)**: All platform price tiers are live across **Web**, **iOS**, and **Android**, plus Steam. Web checkout works out of the box with zero extra configuration.
- **Sandbox Testing in Playground**: In local Playground and preview environments, direct purchases operate in sandbox mode, allowing you to test checkout, order completion, and entitlement grants end-to-end without real charges.

### Standard Platform Price Ladder

The following rungs are pre-provisioned and available across Web, iOS, Android, and Steam:
...
Use `listDirectPurchaseSkus` when you want raw per-store pricing. For rendering
a storefront, prefer `RundotGameAPI.shop.getCatalog()` — it is per item, it
carries names, assets, and sale prices, and it respects your version tag.

```typescript
import RundotGameAPI from '@series-inc/rundot-game-sdk/api'

const skus = await RundotGameAPI.iap.listDirectPurchaseSkus()
for (const sku of skus) {
  console.log(sku.sku, sku.itemId, sku.nominalUsd, sku.grant.entitlementId)
}
```

{% hint style="info" %}
The catalog is always scoped to **your** game. `listDirectPurchaseSkus` takes no
argument — there is no way to list another game's items.

Earlier SDK versions accepted a `gameId` argument. It is now ignored, so
existing code keeps working, but it no longer selects the game and will be
removed in the next major version. Remove it when convenient.

`sku` on each row is the **price-tier id**, not a per-game id, because the
underlying store products are shared across games. Two of your items pinned to
the same tier therefore produce two rows with the same `sku`, distinguished by
`itemId`. That is also why this view cannot replace `shop.getCatalog()` for
merchandising.
{% endhint %}

### `DirectPurchaseSku`

Exported from the package root, `@series-inc/rundot-game-sdk` (not the `/api` subpath), along with `DirectPurchaseStorePrice`, `DirectPurchaseStorePlatform`, and `DirectPurchaseGrantKind`.

<table><thead><tr><th>Field</th><th>Type</th><th>Description</th></tr></thead><tbody><tr><td><code>sku</code></td><td><code>string</code></td><td>The platform <strong>price-tier id</strong> (for example <code>tier_499</code>) — the same value the shop order reports as <code>directPurchase.sku</code>. Store products are shared across games, so this is not unique per item.</td></tr><tr><td><code>itemId</code></td><td><code>string?</code></td><td>The shop item this row was derived from. Use it to tell apart two items pinned to the same tier.</td></tr><tr><td><code>grants</code></td><td><code>DirectPurchaseGrant[]?</code></td><td><strong>Every</strong> entitlement the item grants. Prefer this over <code>grant</code>, which is only the first — a real-money item may grant several things, and rendering just the first understates what the player is paying for.</td></tr><tr><td><code>gameId</code></td><td><code>string</code></td><td>The game this row belongs to.</td></tr><tr><td><code>tierId</code></td><td><code>string</code></td><td>The price tier, same value as <code>sku</code>.</td></tr><tr><td><code>nominalUsd</code></td><td><code>number</code></td><td>Nominal USD price (for display; the store of record sets the actual charge).</td></tr><tr><td><code>storePrices</code></td><td><code>DirectPurchaseStorePrice[]</code></td><td>Per-store pricing entries (see below).</td></tr><tr><td><code>grant</code></td><td><code>{ entitlementId: string; quantity: number; kind: DirectPurchaseGrantKind }</code></td><td>What a completed purchase grants. <code>kind</code> is <code>'consumable'</code> (spent on use, can be bought again) or <code>'durable'</code> (permanent unlock).</td></tr></tbody></table>

### `DirectPurchaseStorePrice`

One entry per store the SKU is sold on. `platform` is `'web' | 'ios' | 'android' | 'steam' | 'telegram'`; the other fields are optional and platform-specific (e.g. `appStoreProductId` on iOS, `playProductId` on Android, `stars` on Telegram). Use the optional `displayPrice` when present for localized price display.
