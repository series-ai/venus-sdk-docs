...
Release notes are authored via the CLI when you deploy. `--changelog` takes the
notes themselves as Markdown, not a path to a file:

```bash
rundot deploy --changelog "## What's new
