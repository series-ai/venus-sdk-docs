...
{% hint style="warning" %}
**Deprecated.** The `sharedAssets` namespace is superseded by the [Asset Library API](ASSET_LIBRARY.md) (`RundotGameAPI.assetLibrary`), which owns shared asset bundles and referenced asset packs. `sharedAssets` routes to the same implementation, so existing calls keep working — but new code should call `RundotGameAPI.assetLibrary.loadAssetsBundle` with the same arguments.
{% endhint %}
...
|------|------------|-------------|
| `burger-time` | `characters` | Character sprites and animations |
| `burger-time` | `burgershop` | Core game assets |
| `burger-time` | `vfx` | Visual effects assets |
