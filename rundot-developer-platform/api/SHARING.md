...
### Crediting creator content (BETA)

If your app hosts pieces made by other people, a share is usually about the
piece, not about your app. Pass a `target` and the share's clicks credit that
piece's author instead of you:

```typescript
await RundotGameAPI.social.shareLinkAsync({
  shareParams: { storyId: 'hero_homecoming' },
  target: { format: 'interactive-fiction', entryId: story.ugcEntryId },
})
```

- `format` is `'interactive-fiction'` or `'video'`.
- `entryId` is your UGC entry id for the piece. It must not contain `":"`.
- The host scopes the entry to your app, so a target can only ever name content
  you host.
- Accepted by `shareLinkAsync`, `createQRCodeAsync`, and `setShareIntentAsync`.

Omit `target` and nothing changes: the share is about your app, as before.

A click only counts for the author when the piece is published, the sharer is
its author, and the person clicking isn't. Everything else is recorded and
ignored.

## Attach share params to the app's Share button (BETA)

`shareLinkAsync` covers shares your game initiates from its own UI. But the
RUN app also renders its own Share button next to your game (in the game
toolbar and the in-game info panel). By default that button shares a plain
link to your game — no params.

Register a **share intent** to change that. While an intent is registered,
the app's own Share button mints a tracked share link carrying your
`shareParams`, exactly as if you had called `shareLinkAsync` at that moment:

```typescript
// The player entered a level — keep the app's Share button in sync:
await RundotGameAPI.social.setShareIntentAsync({
  shareParams: { levelId: 'boss-42' },
  metadata: { title: 'Play Boss Level 42 with me!' },
  slug: 'boss-level-42',
})

// The player went back to the main menu — revert to the plain game link:
await RundotGameAPI.social.clearShareIntentAsync()
```

Recipients receive the params the usual way, via
[`app.resolveLaunchIntent()`](APP.md#launch-intent).

Semantics:

- **Instant for the player.** The app pre-mints the tracked link in the
  background shortly after you set the intent (briefly debounced, so rapid
  updates coalesce into one mint), so tapping Share doesn't wait on the
  network. If the link isn't ready yet (or can't be minted — for
  example, the device is offline), the app shares the plain game link
  instead; the player's share never breaks.
- **Cheap to call.** Your set/clear calls never block on the mint. Call
  `setShareIntentAsync` as often as your shareable state changes; each call
  replaces the previous intent, and consecutive calls with an identical
  payload are deduplicated automatically, so only real changes re-mint.
- **Session-scoped.** The intent applies only while your game is running. It
  is discarded when the game closes; it never affects other games or the
  app's game page after exit.
- **Same shapes and caps as `shareLinkAsync`** for `shareParams`, `metadata`,
  and `slug`. `shareParams` is required — `setShareIntentAsync` rejects
  immediately when it's missing or the caps are exceeded.
- **Playground / mock:** there is no host Share button in those environments,
  so both calls validate and succeed without further effect.
- **Older app versions** don't know these messages, so the calls reject there
  (the standard contract for every SDK RPC — see the error-handling hint at
  the top of this page). Treat a rejection as "the app's Share button keeps
  its default link".
...
## Follow Me Links (BETA)

Send players to the creator's own social profile or Discord server ("follow me on X / Instagram / TikTok" / "join my Discord"). The creator configures these handles once for their account — they apply across all of that creator's games — and your game only ever asks the platform to open a given network. Your game never sees, sets, or handles the underlying handle or URL.

```typescript
const platforms = await RundotGameAPI.social.getConfiguredFollowMePlatformsAsync()

if (platforms.includes('x')) {
  const { completed } = await RundotGameAPI.social.openXFollowMeLinkAsync()
  if (completed) {
    console.log('Opened the creator’s X profile')
  }
}
```

Here is what happens when you call one of the `open…FollowMeLinkAsync` methods:

1. If the creator hasn't configured that platform's handle, the call resolves `{ completed: false, reason: 'not_configured' }` — it never throws. Use `getConfiguredFollowMePlatformsAsync` to decide whether to show a button at all.
2. Otherwise the platform shows a short confirmation sheet, and the player's tap inside it opens the creator's profile (or Discord invite) on that network.
3. The promise resolves `{ completed: true }` once the destination opens, or `{ completed: false }` if the player dismisses the sheet or the open is blocked (for example a popup blocker).

### `getConfiguredFollowMePlatformsAsync(): Promise<FollowMePlatform[]>`

Returns the subset of `['x', 'instagram', 'tiktok', 'discord']` the creator has set a handle for. Feature-detect with this before rendering a "follow us" button so you never show a dead action.

### `openXFollowMeLinkAsync(): Promise<FollowMeResult>`

Opens the creator's X ("Twitter") profile through platform-owned UI. Takes no arguments — the handle is resolved server-side from the creator's account.

### `openInstagramFollowMeLinkAsync(): Promise<FollowMeResult>`

Opens the creator's Instagram profile. Same contract as `openXFollowMeLinkAsync`.

### `openTikTokFollowMeLinkAsync(): Promise<FollowMeResult>`

Opens the creator's TikTok profile. Same contract as `openXFollowMeLinkAsync`.

### `openDiscordFollowMeLinkAsync(): Promise<FollowMeResult>`

Opens the creator's Discord server invite. Same contract as `openXFollowMeLinkAsync`.

`FollowMeResult` is `{ completed: boolean; reason?: 'not_configured' }`. `reason` is present only when `completed` is `false` because the creator hasn't set that platform's handle.
...
| `setShareIntentAsync(options)` | `Promise<void>` | Attach share params to the app's own Share button; pre-mints the tracked link in the background (BETA) |
| `clearShareIntentAsync()` | `Promise<void>` | Clear the share intent; the app's Share button reverts to the plain game link (BETA) |
...
| `getConfiguredFollowMePlatformsAsync()` | `Promise<FollowMePlatform[]>` | List which follow-me platforms the creator has configured (BETA) |
| `openXFollowMeLinkAsync()` | `Promise<FollowMeResult>` | Open the creator's X profile through platform-owned UI (BETA) |
| `openInstagramFollowMeLinkAsync()` | `Promise<FollowMeResult>` | Open the creator's Instagram profile through platform-owned UI (BETA) |
| `openTikTokFollowMeLinkAsync()` | `Promise<FollowMeResult>` | Open the creator's TikTok profile through platform-owned UI (BETA) |
| `openDiscordFollowMeLinkAsync()` | `Promise<FollowMeResult>` | Open the creator's Discord server invite through platform-owned UI (BETA) |
