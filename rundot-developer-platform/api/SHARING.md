...
Send players to the creator's own social profile ("follow me on X / Instagram / TikTok"). The creator configures these handles once for their account — they apply across all of that creator's games — and your game only ever asks the platform to open a given network. Your game never sees, sets, or handles the underlying handle or URL.
...
2. Otherwise the platform shows a short confirmation sheet, and the player's tap inside it opens the creator's profile on that network.
3. The promise resolves `{ completed: true }` once the profile opens, or `{ completed: false }` if the player dismisses the sheet or the open is blocked (for example a popup blocker).

### `getConfiguredFollowMePlatformsAsync(): Promise<FollowMePlatform[]>`

Returns the subset of `['x', 'instagram', 'tiktok']` the creator has set a handle for. Feature-detect with this before rendering a "follow us" button so you never show a dead action.
