...
On RUN app web and native hosts, insufficient funds automatically opens the currency purchase flow. After a completed top-up, the host retries the original Shop order once with the same idempotency key. Dismissing the top-up rejects with a `RundotApiError` whose code is `USER_CANCELLED`; a completed top-up that still does not cover the order rejects with `insufficient_funds`. The Playground HTTP host instead surfaces `insufficient_funds` directly so you can fund the test wallet with `RundotGameAPI.iap.openStore()`.
