...
|---|---|
| `bucks` | RUN Bits (platform hard currency) |
| `direct_purchase` | Direct FIAT purchase, charged through the player's app store |

The `value` field is a string to support decimal precision for direct purchases.

For a `direct_purchase` item the order does not complete in one call. `purchase()`
returns an order carrying a `directPurchase` block, the host runs store checkout,
and the order reaches `fulfilled` only after the charge is verified server-side:
...
|---|---|---|
| `sku` | string | The direct-purchase SKU the order is priced against |
