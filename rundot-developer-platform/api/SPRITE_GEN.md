...
const tile = await RundotGameAPI.spriteGen.generate({
  ...
  mode: 'texture',       // 'assets' (default) | 'texture' (seamless/tileable) | 'ui'
...
`mode: 'texture'` is the path to seamless/tileable output (the base input for tileset pipelines). `resolution`/`quality` trade cost against fidelity per asset class; the effective values are echoed on the result.
...
| `mode` | `'assets' \| 'texture' \| 'ui'` | Generation mode; `'texture'` yields seamless/tileable output (default: `'assets'`) |
