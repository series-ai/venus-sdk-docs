...
{% hint style="info" %}
**Use the SDK storage APIs below for player data.** `deviceCache`, `appStorage`, `ownerStorage`, and `sharedStorage` are the supported ways to persist state; they work consistently across web and mobile and are scoped to the right sharing model. Browser storage APIs (`localStorage`, `sessionStorage`, `IndexedDB`, cookies, etc.) are not available inside the game iframe (see [Runtime Environment](../runtime-environment.md)). The SDK's Vite plugin includes a build-time and dev-time linter (`storageCheck`) that catches direct browser storage calls automatically.
{% endhint %}
...
`appStorage.compareAndSwap(key, expectedValue, nextValue)` changes a key only when its current stored value equals `expectedValue`, atomically:

- `expectedValue: null` requires the key to be absent (create-only).
- `nextValue: null` deletes the key.
- Resolves `{ swapped: true }` when the change was applied.
- Resolves `{ swapped: false, currentValue }` when the stored value didn't match. A conflict is **not** an error and never changes stored data.
- Auth, validation, quota, rate-limit, and transport failures reject as usual — a rejection is always distinct from a conflict.

```typescript
// Commit a new version of a data model: write new immutable records, then
// swap the head. Retry the whole prepare-and-swap cycle on conflict.
const head = await RundotGameAPI.appStorage.getItem('head')
await RundotGameAPI.appStorage.setMultipleItems(newRecords)
const result = await RundotGameAPI.appStorage.compareAndSwap('head', head, nextHead)
if (!result.swapped) {
  // result.currentValue holds the value another writer committed first
}
```

Use it to commit a new version of a data model: write new immutable records with `setMultipleItems`, then swap a small head record. Embed a unique commit identifier (e.g. a UUID) in the head value so an old head can never become valid again (ABA). After an ambiguous failure (timeout, offline), re-read the head to learn whether your swap landed.

Hosted writes are buffered (~100 ms). `compareAndSwap` flushes your pending writes before swapping, so the compare always runs against your own latest state. It is never retried automatically, and while the storage rate limiter is backing off it fails fast locally instead of queueing — retry after `retryAfterMs`.

`compareAndSwap` is available on `appStorage` only (hosted, playground, and mock) — not on `ownerStorage`, `sharedStorage` handles, or `deviceCache`.

## Reading Many Keys

`appStorage.getMultipleItems(keys)` reads up to 100 keys in one request and resolves a map of the keys that exist — missing keys are simply absent. Duplicate keys are read once; an empty list resolves `{}` without a request. Any invalid key rejects the whole request. Hosted response size is bounded by the bucket's byte cap (10 MiB by default); the playground has no aggregate bucket cap, so there the bound is 100 keys × the per-value limit. There is no snapshot isolation across keys — for content-addressed records that's fine, because immutable records never change and the atomic head selects exactly which keys belong to a version.

```typescript
const records = await RundotGameAPI.appStorage.getMultipleItems(['rec_a', 'rec_b', 'rec_c'])
// → { rec_a: '…', rec_c: '…' }  (missing keys omitted)
```

Like `compareAndSwap`, `getMultipleItems` is available on `appStorage` only.
...
Mutation validation is atomic. If any key or value in a batch is invalid, the call rejects with `INVALID_ARGUMENT` before any valid-looking member is written.
...
| Affected storage data per mutation | 8 MiB |
| Items per batch call | 400 |
| Keys per `getMultipleItems` call | 100 |
| Key size (UTF-8 bytes) | 256 |

A mutation's affected-data estimate includes existing key/value data being
deleted or replaced plus new key/value data being written. If a clear,
clear-first replacement, or incremental batch would exceed the 8 MiB mutation
budget or Firestore's single-transaction write limit, the call rejects with
`PAYLOAD_TOO_LARGE` before any writes are committed. Existing bucket data
remains intact. Split incremental work into smaller calls; a quota override
does not raise this per-mutation limit.

Values above **256 KiB** are accepted but logged as a soft-limit warning server-side; treat 256 KiB as the comfortable working size and ~977 KiB as the hard ceiling (each value is stored individually, bounded by a ~1 MiB per-value limit). For larger or binary payloads use the [Files API](FILES.md).
...
Content-addressed models (see [Atomic Compare-and-Swap](#atomic-compare-and-swap)) store many immutable records in one bucket. The default bucket cap is 128 items / 10 MiB — if your record graph outgrows it, request a per-app quota override rather than packing records into fewer keys.
...
| `PAYLOAD_TOO_LARGE` | One mutation exceeds the 8 MiB affected-data budget or the single-transaction write limit; no bucket changes are committed. |
