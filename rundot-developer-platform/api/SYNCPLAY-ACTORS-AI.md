...
> **PROVISIONAL:** This module API may change without deprecation.
> Two genre examples, ARPG and Beat-em-up, already consume it directly. The API
> does not wait for a second consumer. It waits for a dedicated promotion review
> of its recorded semantic decision.
...
`navmesh` is a `CookedDeterministicNavmesh`. Build it with
`cookDeterministicNavmesh` for an authored source, or with
`cookImportedDeterministicNavmesh` for an imported mesh. `avoidanceObstacles`
is an optional list of `DeterministicNavmeshMovingAvoidanceObstacle`. Both types
and both cook functions come from the root entrypoint. See
[Syncplay: Pathfinding](SYNCPLAY-PATHFINDING.md).
...
`DeterministicNavmeshMovingAvoidanceObstacle` shape and are consumed by the live
crowd step. See [Syncplay: Pathfinding](SYNCPLAY-PATHFINDING.md).
