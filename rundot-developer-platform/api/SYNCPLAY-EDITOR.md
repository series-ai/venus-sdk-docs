...
never throws at the sender.

**The client counter sees only one of those five causes.** Read it at
`snapshot.netStats?.rejectedCommands` — `netStats` is a field on the snapshot and
is `null` offline. It increments **only** when this client refuses to send an
oversized payload locally. Rate-limit overruns, replayed sequence numbers, forged
slots, and stale-generation drops all happen inside the authority. They increment
the authority's own counter (`AuthorityRoomNetcodeStats.rejectedCommands`, read
through `netcodeStats()` on the room) and are never sent to a browser client.

{% hint style="warning" %}
Do not build a "your edit didn't land" affordance on
`snapshot.netStats?.rejectedCommands`. Four of the five drop causes can never
raise it, so the affordance would stay silent in exactly the cases that matter.

Detect a lost edit by **observing the expected state change**: after
`submitCommand`, record the cell you edited, then watch confirmed frames for the
matching `state.tileOverrides` value. If it does not arrive within a timeout of
your own choosing (a few round trips), treat the edit as dropped and re-submit or
warn. The client counter is still useful for one thing: it tells you immediately
that your payload was too large to send.
{% endhint %}
