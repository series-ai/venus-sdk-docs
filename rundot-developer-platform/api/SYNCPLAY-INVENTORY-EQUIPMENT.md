...
<!-- syncplay-example: inventory-equipment-imports -->
```ts
import {
...
state throws `SYNCPLAY_INVENTORY_EQUIPMENT_SLICE_INVALID`.
`pickupFromSyncplayLootResult` throws
`SYNCPLAY_INVENTORY_EQUIPMENT_LOOT_INVALID` for a malformed loot result or
pickup ID. These failures occur before the caller's slice is mutated.
...
The stats/abilities module has not passed the two-genre promotion gate. Its API
can change without a deprecation period. `stepInventoryEquipmentWithStats` is
therefore only as stable as that module. `stepSyncplayInventoryEquipment` has no
such dependency. Use it when you must not inherit that risk.
...
not roll again or generate an ID. A malformed result or pickup ID throws
`SYNCPLAY_INVENTORY_EQUIPMENT_LOOT_INVALID`.
