...
For a data-authored side-view platformer, start with the installed Kinetix
platformer pack in [UGC Platformer Preset](SYNCPLAY-PLATFORMER.md). Use these
lower-level movement primitives when the installed vocabulary does not fit;
they are not a second platformer session/runtime framework.
