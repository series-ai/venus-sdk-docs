...
throw; a hull may not be a `compound` child). Hulls collide against boxes,
spheres, capsules, other hulls, and static meshes, and participate in every query
(raycast/overlap/shape-cast). Rotational inertia uses the vertex AABB
approximation (same treatment as compound bodies).
