...
`@series-inc/rundot-syncplay/modules/platformer-2d` is the only supported
platformer. The package also ships an internal WASM-backed twin at
`core/src/physics/platformer-2d.mjs`. No `exports` subpath reaches that file.
Do not import it.

The two files use names that look almost the same. The supported module exports
`PLATFORMER_2D_DEFAULT_CONFIG` and `createPlatformer2DSlice`. The internal twin
exports `PLATFORMER_2D_CONFIG_STRIDE` and `createPlatformer2DState`. Read each
identifier before you copy it.

Import the generic, code-only platformer controller from:

<!-- syncplay-example: platformer-2d-step -->
```ts
