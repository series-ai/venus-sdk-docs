...
### Projection (`KinetixPlatformer2dProjection`)

What `runtime.project()` returns each frame — i.e. what the runner's
`presentation.project(projection, …)` receives. Entries are frozen sim objects
shared with internal state: cheap to read, and they must never feed back into
simulation. A player object also carries sim-internal timers (coyote/warp
frames and the like); anything not listed below is internal and not covered by
the reducer ABI. The typed contract is `runtime.d.ts`; this table is the
human-readable mirror.

| Field | Shape |
|---|---|
| `frame`, `phase`, `phaseFrame` | Current frame; `'countdown' \| 'playing' \| 'results'`; frames elapsed in that phase |
| `players[]` | `{ slot, x, y, vx, vy, health, maxHealth, grounded, groundId, dead, finished, score, coins, facing: -1 \| 1, crouching, climbing, swimming, groundPounding, carryingId, checkpointId, doubleJumpCharges }` |
| `platforms[]` | `{ id, type: 'movingPlatform' \| 'fallingPlatform', x, y, previousX, previousY, waypointIndex, waypointDirection, legFrame, vy, fallState, armedFrames, fallFrame }` |
| `stompables[]` | `{ id, alive, respawnFrames, graceFrames }` |
| `enemies[]` | `{ id, type: 'patrolEnemy' \| 'waypointEnemy', x, y, previousX, previousY, vx, vy, facing, grounded, alive, respawnFrames, graceFrames, waypointIndex, waypointDirection, legFrame }` |
| `hazards[]` | `{ id, x, y, previousX, previousY, waypointIndex, waypointDirection, legFrame }` — `movingHazard` waypoints only |
| `pickups[]` | `{ id, collected }` — level-authored coins and power-ups |
| `spawnedPickups[]` | `{ id, kind, x, y, halfWidth, halfHeight, collected }` — bonus-block spawns mid-match |
| `carryItems[]` | `{ id, x, y, previousX, previousY, vx, vy, mode: 'idle' \| 'carried' \| 'thrown', carrierSlot: -1 \| slot }` |
| `extensions[]` | `{ id, state }` — your `behavior.init` return values, sorted by entity id |
| `tileOverrides` | Sparse `[tileIndex, tileId]` pairs from broken/spent blocks; fold in with `applyKinetixPlatformer2dTileOverrides` (or read directly) before drawing tiles |
| `events[]` | See [Event vocabulary](#event-vocabulary) |

Interpolate between consecutive render states with `presentation.interpolate`;
`previousX`/`previousY` on entries are sim internals, not an interpolation API.
...
import { encodeKinetixPlatformer2dLevel } from '@series-inc/rundot-syncplay/core/runtime'
...
The default cap leaves **no headroom for the record envelope**:
`publishPlatformerLevel` nests the level at `data.level` alongside the indexed
fields (`name`, `authorSlot`, `playerCapacity`, `version`, `thumbnailAssetId?`)
and the JSON keys themselves, so the stored record is level bytes plus roughly
100–200 bytes — a max-size level is rejected by `ugc.create`. Budget levels at
≤ 101 KiB canonical and enforce it at publish time by encoding with a lower
`maxBytes` (`encodeKinetixPlatformer2dLevel(level, { maxBytes: 101_376 })`), so
the editor fails loudly instead of the create call; or raise your app's UGC
`data` cap.
...
and must therefore move runtime identity: an extended registry must ship a
`registryManifest` — the `kinetixRuntimeModuleManifest` hash list covering every
extension module, including transitive imports — so the extension source is
folded into `engineIdentityHash`.
