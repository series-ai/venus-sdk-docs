...
The Syncplay core owns the deterministic contract — checkpoints, rollback, checksums,
...
| Shape | Use it when | Syncplay core owns | Your game owns |
|---|---|---|---|
...
Start with a generic module when one fits. Otherwise use
...
<!-- syncplay-example: runtime-composition-session-schema -->
```ts
import type { KinetixSessionConfigSchema } from '@series-inc/rundot-syncplay/core'
...
game-owned module list and the exact dependency version is folded into
`engineIdentityHash`:
...
export const runtimeIdentity = deriveCustomRuntimeIdentity({
  ...
  deterministicVersion: `my-game/v1+syncplay@${syncplayVersion}`,
  sessionConfigSchema: counterSessionSchema,
})
```

The Syncplay core owns canonical hashing and identity validation. Your build owns the
complete module list and the exact version. A missing runtime module or a
...
malformed module list throws `KINETIX_PRODUCT_RUNTIME_MODULES_INVALID`. Bumping
the `@series-inc/rundot-syncplay` dependency changes `engineIdentityHash`, so
an upgrade forces regeneration instead of silently reusing the old hash.

## 2. Adapt immutable game state (the authoritative path)

This recipe is the authoritative path. The core calls your `step`, then
`captureState`, then hashes what `captureState` returned. That hash is the
checksum the peers compare. A value is therefore authoritative if, and only if,
`step` wrote it into the captured state. Every deterministic API is legal inside
`step`, and a result you keep out of captured state stays cosmetic. The full
rule is in [What authoritative means
here](SYNCPLAY.md#what-authoritative-means-here).
...
The Syncplay core owns opaque checkpoint handles and atomic rollback: a failed batch
restores the starting state and retains no partial checkpoints. Your game owns
the immutability guarantee.

**`capturedStateIsDetached` is a correctness promise, not a performance dial.**
It tells the adapter that no code mutates a captured state root after capture,
so the adapter retains that root by reference instead of cloning it. Keep the
promise and capture stays O(1). Break the promise — set the flag on state you
mutate in place — and every checkpoint aliases the live state. A rollback then
restores the present instead of the past, the run keeps going, and the fault
surfaces many frames later as a desync. Nothing throws, and no error names the
flag. If your state mutates in place, omit the flag. The adapter then clones
each captured state, which costs one clone per tick and is correct.

Validate inputs inside `step` (here `readDelta` enforces the
...
The Syncplay core owns checkpoint lookup and passes retained state read-only; an absent
...
The Syncplay core re-serializes hydrated state and rejects byte drift as
...
The Syncplay core decodes the complete batch before stepping and maps any decoder failure
...
and its limits. The decoder above reads the shape a caller passes to
`session.recordCommands()` directly. A command sent with
`runner.submitCommand(kind, payload)` instead arrives wrapped as
`{ kind: 'game.<kind>', slot, payload }`, so decode that envelope when the
runner owns the session (recipe 7). Omit the hook for `{ commands: false }`; non-empty command
...
The Syncplay core owns digest and length verification — a mismatch throws
...
in `error` before frame 0 exists.

`SyncplayRunnerConfig` carries no command channel. There is no config field that
sends a command. Send one with `runner.submitCommand(kind, payload)`. Networked,
the client hands it to the authority, which orders it and returns it to every
peer inside the confirmed frame. Offline, the runner applies the same envelope
on the next stepped frame. Both paths deliver
`{ kind: 'game.<kind>', slot, payload }` to your `decodeCommand` hook, so the
runtime must declare that hook (recipe 5) or the batch fails with
`KINETIX_RUNTIME_COMMAND_INVALID`. When you drive a session yourself instead of
a runner, `session.recordCommands()` is the equivalent entry point. The full runner contract — snapshot fields,
...
<!-- syncplay-example: runtime-composition-proof-imports -->
```ts
import {
  assertDeterministic,
