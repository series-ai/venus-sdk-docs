...
```json
{
  "rooms": [{
    ...
    "config": { "playerCount": 2, "maxPlayers": 2 }
  }]
}
```

A secret-enabled room must declare `config.playerCount`. Secret systems are
sized against the simulation slot count, and `config.maxPlayers` can be lowered
for an individual room by a join ticket.
...
Confirmed authority commands reach your simulation through
`createInstalledRuntimeAdapter`. Its step hook takes **two** arguments,
`step(inputs, commands)`. It receives no state, no frame, and no context object:
your game owns state through `captureState`, `restoreState`, and `frameOf`. Pass
`{ commands }` to `createSyncplaySecretEventReader`.

```ts
import { createSyncplaySecretEventReader, type CanonicalValue }
  from '@series-inc/rundot-syncplay/browser'
import { createInstalledRuntimeAdapter } from '@series-inc/rundot-syncplay/core/runtime'

let state: GameState = createInitialState()

const runtime = createInstalledRuntimeAdapter<GameState, Input, CanonicalValue>({
  identity: runtimeIdentity,
  sessionConfigBytes,
  captureState: () => state,
  restoreState: (saved) => { state = saved },
  frameOf: (saved) => saved.tick,
  // Required. Without it the adapter rejects every non-empty command batch
  // with KINETIX_RUNTIME_COMMAND_INVALID, so no secret event ever arrives.
  // A pass-through is enough: the authority already canonicalized the command.
  decodeCommand: (command) => command as CanonicalValue,
  step: (inputs, commands) => {
    const events = createSyncplaySecretEventReader({ commands })
    let next = state
...
{% hint style="warning" %}
`decodeCommand` is optional in the type, but a runtime built without it cannot
accept commands at all. The adapter fails the **whole batch** with
`KINETIX_RUNTIME_COMMAND_INVALID` before any frame steps. Every secret-enabled
game must supply it. Any throw inside `decodeCommand` produces the same code, so
validate there if you also accept game-authored commands.
{% endhint %}
