...
<!-- syncplay-example: stats-abilities-imports -->
```ts
...
`createDeterministicMath()` uses a default `fixedScale` of `1000`. A multiply
modifier is therefore a permille factor. `value: 250` multiplies the stat by
`0.25`. `value: 1000` leaves the stat unchanged. `value: 1500` multiplies it by
`1.5`. An add modifier uses raw stat units and is not scaled.
...
The standalone helpers throw two more codes.
`recomputeSyncplayStatBlock` and `replaceSyncplayStatModifiersBySource` throw
`SYNCPLAY_STAT_MODIFIER_INVALID` for an invalid math object, base block, or
modifier list. `resolveSyncplayLootRoll` throws `SYNCPLAY_LOOT_ROLL_INVALID`
for an invalid math object, loot table, or roll.
...
input. `resolveSyncplayLootRoll(table, roll, math)` maps that ticket to a
canonical entry. This is deterministic loot resolution, distinct from platform
Stats analytics and from Simulation/Gacha service configuration.

The helper takes three arguments and validates `math` first. A two-argument
call throws `SYNCPLAY_LOOT_ROLL_INVALID`.

```ts
const result = resolveSyncplayLootRoll(
  { id: 'chest', entries: [{ id: 'gold', itemId: 'gold', quantity: 5, weight: 1 }] },
  { rollId: 'roll-1', tableId: 'chest', ticket: 0 },
...
- `createSyncplayStatsAbilitiesSlice`
- `stepSyncplayStatsAbilities`
- `applySyncplayStatsAbilitiesOrderedEffects`
- `projectSyncplayStatsAbilities`
- `statsAbilitiesComponent`
- `statsAbilitiesModuleApiStability`
