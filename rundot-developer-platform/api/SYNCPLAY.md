...
| `matchLog` | `false` | Set `true` to keep the full server-ordered input log so finished matches can be **re-simulated offline for score verification** (`verifyScoredMatchLog`) — the anti-cheat basis for verified leaderboards. |
...
The scaffolder wires these into `package.json` scripts; the key commands:

Venus validates **two** artifacts on upload — the deploy-check and its
verification — and `project:deploy-check` consumes a release bundle, so run the
full chain (the scaffolder wires these into `package.json` build scripts):

```bash
# 1. Prove the offline simulation is deterministic; emit the release evidence
npx rundot-syncplay project:check syncplay.project.json --require core --frames 32 \
  --dist dist --release-json artifacts/deterministic/project-release-evidence.json

# 2. Bundle the release evidence
npx rundot-syncplay project:release-bundle artifacts/deterministic/project-release-evidence.json \
  --json artifacts/deterministic/project-release-bundle.json

# 3. Mint the deploy-check proof (add --require-browser-proof for the browser gate;
#    otherwise a headless proof is minted)
npx rundot-syncplay project:deploy-check syncplay.project.json \
  --bundle artifacts/deterministic/project-release-bundle.json \
  --json artifacts/deterministic/project-deploy-check.json

# 4. Emit the verification artifact the server also requires
npx rundot-syncplay project:deploy-check-verify artifacts/deterministic/project-deploy-check.json \
  --json artifacts/deterministic/project-deploy-check-verification.json
```
  ...
  "rooms": [
    {
      ...
      "deterministic": {
        ...
        "deployCheckVerificationPath": "artifacts/deterministic/project-deploy-check-verification.json"
      }
    }
  ]
}
