# Syncplay: deterministic multiplayer (BETA)
...
Run `npm view @series-inc/rundot-syncplay dist-tags`, take the greater of
`latest` and `beta` (it must be **>= 5.26.0**), and install that exact version.
Expect npm to raise `@series-inc/rundot-game-sdk` to satisfy the peer range;
that bump is correct. Then verify the subpaths resolve before writing code:
...
export const runtimeIdentity = deriveCustomRuntimeIdentity({
  ...
  deterministicVersion: 'my-game@1+syncplay5.26.0',
...
dependency change requires regeneration. The version string is folded into `engineIdentityHash`, so the old
