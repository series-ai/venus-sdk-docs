...
provide `config.checksum: state => state.hash`. This is the intended O(1) seam
for regular desync checks. Late-join and promotion snapshots still serialize and
checksum the complete state on demand; the wire checksum is deliberately not
