...
The **transport** owns reconnection, not the runner. A `'reconnecting'` transport
event is non-terminal: the snapshot reports `connection: 'reconnecting'`, the
runner resumes via presentation catch-up, and `status` returns to `live`. A hard
`'disconnected'` is terminal and lands `status: 'error'`.

The runner deliberately adds **no** re-dial machinery — re-invoking an opaque
connect factory risks a fresh join rather than reclaiming the seat. Offer the
player a re-join action on terminal disconnect instead.
