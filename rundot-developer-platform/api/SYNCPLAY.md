...
**`maxSnapshotBytes`.** If your state serializes above the 4 MiB default
(destructible worlds, large entity counts), raise the ceiling on **both** sides
— `maxSnapshotBytes` in the room's `realtime.config.json` `config` block and
`maxSnapshotBytes` on `createSyncplaySession` / `createNetworkedSyncplayClient`:

```typescript
const session = createSyncplaySession<State, Input>({
  descriptor, initialState, defaultInput: NEUTRAL, step,
  encodeInput: packInput, decodeInput: unpackInput,
  maxSnapshotBytes: 16 * 1024 * 1024,   // must match the room's configured ceiling
})
```

The room and the client are configured independently, and each rejects an offer
above **its own** ceiling with `snapshot-too-large`. A client ceiling below the
room's snapshot size therefore breaks late-join silently — keep the two values
equal.
...
| `maxSnapshotBytes` | `4194304` (4 MiB) | Per-room snapshot transfer ceiling in bytes. Raise it when your state legitimately serializes above 4 MiB (large destructible worlds, big entity counts). **Set the same value in `SyncplaySessionConfig.maxSnapshotBytes` on the client**: each side rejects an offer above *its own* ceiling with `snapshot-too-large`, so a mismatch silently breaks late-join. |
