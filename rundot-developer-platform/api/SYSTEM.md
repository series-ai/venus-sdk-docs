...
| Method | Returns | Use |
| --- | --- | --- |
| `canFullscreen()` | `boolean` | Synchronous helper returning true when `capabilities.fullscreen === 'toggleable'`. |
...
Use `canFullscreen()` to check if the player can toggle fullscreen:

```typescript
if (RundotGameAPI.system.canFullscreen()) {
  // Show fullscreen toggle button
}
```

Or inspect `capabilities.fullscreen` directly on the environment:
...
> [!NOTE]
> In local development (`MockHost` and `PlaygroundHost`), `capabilities.fullscreen` defaults to `'unavailable'` because mock harnesses lack host window arbitration. `canFullscreen()` returns `false` in mock environments unless capabilities are explicitly overridden.
...
The RUN.world toolbar stays visible throughout and says what the next Escape will
