...
**Idempotency.** Pass a stable `clientRequestId` if you want to safely retry a vote after a network failure — the server returns the cached result instead of double-counting:

```typescript
await RundotAPI.ugc.voting.vote({
  entryId,
  count: 1,
