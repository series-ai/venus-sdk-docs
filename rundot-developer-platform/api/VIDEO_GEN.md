...
The platform serves video models from a runtime registry — new models become
available without an SDK release. The models available at time of writing:

| Provider | Best For |
|----------|----------|
| `seedance-2.5` | High-quality generation with reference support, audio generation, draft preview, 4–30s |
| `seedance-2.0` | High-quality generation with reference support, audio generation, 4–15s |
| `seedance-2.0-fast` | Faster generation at slightly lower quality, same capabilities as 2.0. 480p/720p only |
| `seedance-2.0-v2-fast` | Seedance 2.0 enterprise/v2 fast tier, same parameters as `seedance-2.0-fast`. 480p/720p only |
| `kling-3.0-standard` | Multi-prompt storytelling, negative prompts, configurable shot types, 3–15s |

The SDK's `VideoGenProvider` type accepts any string so a registry addition
type-checks before the next SDK release; the four ids above remain as
autocomplete hints.

No current provider supports `cameraFixed`, and passing it is **rejected**
with `VIDEO_GEN_INVALID_PARAMS` — remove it rather than relying on it being
ignored. `negativePrompt` is accepted by Kling and silently dropped by
Seedance, which declares no such field.
...
For `videoGen`, `allowedModels` entries match the request's **`provider`** (e.g. `seedance-2.0`, `seedance-2.0-fast`, `seedance-2.0-v2-fast`, `kling-3.0-standard`), **not** a model id. Putting a raw model id in the list silently fails to match every request and blocks all calls — list the `provider` values instead.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `provider` | `VideoGenProvider` | Required: a video model id from the platform registry (any string; the four known ids autocomplete). Unknown ids are rejected by the server with `VIDEO_GEN_INVALID_PARAMS` |
...
| `draft` | `boolean` | Optional: request fast low-resolution (480p) draft preview on supported models (e.g. `seedance-2.5`). Bills at the draft rate and returns a `draftId` for subsequent 1080p completion |
...
| `durationSeconds` | `number` | Video length in whole seconds. The accepted range is a property of the **model**, not the SDK — read `durationRange` for your provider/mode from `GET /v1/videogen/models`. Omit to let the provider pick its own default |
| `generateAudio` | `boolean` | Generate synchronized audio track. Defaults to `true` — pass `false` explicitly for silent output |
...
| `durationSeconds` | `number \| undefined` | The requested duration. Absent when the caller named none — the provider chose it and reports none back |
| `width` | `number \| undefined` | Video width in pixels. Absent: current providers do not report frame dimensions |
| `height` | `number \| undefined` | Video height in pixels. Absent: current providers do not report frame dimensions |
| `provider` | `VideoGenProvider` | Provider that generated the video |
| `mode` | `VideoGenMode` | Mode used for generation |
| `prompt` | `string` | The prompt used |
| `seed` | `number \| undefined` | The seed the provider reported (Seedance). Absent for providers that report none (Kling) |
| `draftId` | `string \| undefined` | Identifier for draft generations, used to complete/upgrade drafts within 7 days |
