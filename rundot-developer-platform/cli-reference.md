...
`rundot` is the command-line tool for shipping and operating a RUN.game title:
...
Beta-gated groups: `marketing`, `socials`, `analytics`, `ugc`, `stats`,
...
| `rundot socials …` | Organic launch packets (beta) — see [Marketing Your Game](marketing-your-game.md#share-an-update-launch-packet) |
...
- **Edit slot** (structure-preserving reskin anchor, choose one): `--edit-file`, `--edit-file-key` (creator-storage key), or `--edit-asset-id`.
...
- `--pixel` enables pixel-art mode; `--pixel-perfect` (default `true`) snaps to grid; `--smart-crop` (default `true`) crops to content bounds.
- `--colors "#1a1a2e,#16213e,…"` locks a palette (max 8 hex; watch for palette bleed on subtle hues).
...
rundot leaderboard reset  <leaderboard-id> --confirm      # DESTRUCTIVE — wipe all scores
...
- **Ad copy is campaign-level**, authored once in `campaign.json` (`headlines` /
  `primaryTexts`, ≤ 5 each). The per-creative sidecars carry prompt/seed/refs
  only — there is **no `marketing copy` command**.
...
rundot socials promo --platform x --name "My Game"   # platform-sized promo image
...
Market intelligence for paid creators, priced in RUN.game credits. Free actions
...
- `rundot analytics export <query> --game-id <id>` / `rundot analytics queries` — pre-approved creator analytics exports (beta).
