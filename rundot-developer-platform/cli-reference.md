...
Default model is `gemini-3.1-flash-image-preview`. Available models include `gemini-3.1-flash-lite-image`, `gemini-3-pro-image-preview` (the only model supporting `--image-size` `1K`/`2K`/`4K`), and OpenAI models (`gpt-image-2`, `gpt-image-2.5-flare`, `gpt-image-2.5-sunburst`, requiring `power` tier). Run `rundot generate image-models` to query callable models for your account. Background removal models:
`bria` (fast, default) or `birefnet` (high quality; tune with
`--remove-background-variant` and `--remove-background-resolution`).

`--reference-image` accepts local file paths, `https://` URLs, and `data:` URIs with or without a `--game-id` (local files are encoded in-memory and transiently uploaded with the generation request). Creator-storage file keys require `--game-id`. Up to 10 reference images (max 10 MB per file) may be supplied.
