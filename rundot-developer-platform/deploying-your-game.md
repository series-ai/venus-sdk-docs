...
For headless **playground** sandbox login — no Google Sign-In, nothing embedded
...
instead: it can *only* open a sandbox session and is rejected (403) by every
...
This writes a git-ignored, `0600` `.env.local` containing `RUNDOT_PLAYGROUND_KEY=rk_…`
...
`rundot playground revoke-access`. The key validates against RUN.game, so it
...
| Key type | Two distinct, prefix-typed credentials: **deploy** (`rk_`, minted by `api-keys create` / this REST endpoint — full game access) and **playground** (`pk_`, minted by `rundot playground grant-access` — sandbox-login only, rejected by every mutation/spend route). The type is intrinsic to the key's prefix. |
...
> **Beta.** `--changelog` and `--major` are currently beta features. They're
...
By default a deploy is a **minor** release: the changelog shows up on the About
tab, but nobody is actively notified. When you ship something worth shouting
about, mark it **major**:

```bash
rundot deploy --public --major --changelog "## Season 2 is here!
A brand-new campaign, 12 levels, and online co-op."
```

A **major public** release notifies your existing fans — players who liked your
game get a message in their in-app inbox, and dormant players may receive a
push notification (subject to a per-game weekly cap so they're never spammed).
A **minor** release only updates the release-notes display.

{% hint style="info" %}
Only **major** releases that are **public** notify fans. A `--major` deploy
without `--public`, or a major bump to an unlisted build, updates the release
notes but does not notify anyone.
{% endhint %}
...
* `--major`: Mark a major release — a major **public** release notifies fans via inbox + dormant push; minor (default) only updates the release notes
