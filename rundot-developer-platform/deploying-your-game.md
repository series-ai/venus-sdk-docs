...
```bash
rundot playground grant-access                        # 365-day CLI default
...
| Expiration | 1–365 days (default **365** for both `api-keys create` and `playground grant-access`; override with `--expires-in-days`) |
...
| Max per game | 10 **active** keys (unrevoked and unexpired; expired keys no longer consume a slot) |
...
Every submission must be a new version, higher than the one that is already public (like an app store update). `rundot deploy` bumps the version for you; submitting the version that is already live is rejected. To ship a change to a live game, run `rundot deploy --public` again.
...
4. Updates the `dev` release to point to the new version
5. Optionally sets the version as public (visible on Explore)
6. Returns OneLink URLs for both public and unlisted access

### Device Pixel Ratio (DPR) Clamping & Mobile Performance

By default, RUN.world automatically clamps `window.devicePixelRatio` to `1.5` on mobile devices (iOS & Android, covering native app webviews and mobile web browsers) via the `"maxDpr"` setting.

**Why DPR clamping is enabled by default:**
- Modern mobile screens routinely have DPR 2.6x–3.5x ($1260 \times 2800$ physical pixels). Full-resolution rendering forces mobile GPUs to allocate massive 4K backbuffers, causing severe thermal throttling, fill-rate stalls, and sub-30 FPS frame drops on mid/low-tier Android chipsets.
- On iOS devices, excessive canvas memory allocation triggers WebKit Jetsam process termination (`onContentProcessDidTerminate`), crashing the game session abruptly.
- Clamping DPR to `1.5` reduces GPU backbuffer allocation and fill-rate by ~75%, stabilizing frame rates at 60 FPS while remaining visually crisp at standard mobile viewing distances.

**Customizing or Uncapping `maxDpr`:**
You can customize the maximum DPR ceiling by setting `"maxDpr"` in `game.config.prod.json`:
...
- **`1.5` (default)**: Recommended for optimal battery, thermals, and stable 60 FPS on mobile GPUs.
- **`2.0`**: Crisp high-density resolution for lighter 2D canvas games or games with fine vector typography.
- **`3.0` (or native)**: Leaves `window.devicePixelRatio` untouched at the device's native resolution.
