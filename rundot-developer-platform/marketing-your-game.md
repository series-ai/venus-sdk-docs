...
Run paid ad campaigns for your game straight from the `rundot` CLI. You write a short prompt, RUN.world generates the source ad creatives for you, and a campaign is submitted to Meta (Facebook / Instagram) to drive new players to your game.
...
.rundot/marketing/spring-push/
  ...
  square/                # 1:1 source creatives (generated by default)
  vertical/              # 9:16 source creatives (opt-in via --kind vertical)
  landscape/             # 1.91:1 source creatives (opt-in via --kind landscape)
...
| Option | Purpose |
| --- | --- |
| `--platforms web,ios,android` | Which platforms to advertise on. **Omit to run web only** — app-install (iOS/Android) is an explicit opt-in. |
...
| `--daily 35` | Initial per-leg daily budget in USD ($5–$500). Omit for the network default ($50/day app, $20/day web). |
...
For every campaign type, `prepare` warns when the resolved per-leg daily budget is at or below ~$50/day and may gather too little signal during Meta's learning phase. After creatives exist, `generate` separately warns when the budget falls below ~$15/creative/day.
...
| Option | Purpose |
| --- | --- |
| `--kind square` | Generate only one source-creative kind (`square`, `vertical`, `landscape`, `logo`). Omit to generate `logo` + `square` only. |
| `--variants 4` | How many variants to generate (default 4 for `square`, 3 otherwise; capped per kind). |
...
Before a paid square or landscape generation starts, the CLI preflights every reference image with OCR. Detected text produces a warning but does not block the run; use text-free references so the generated output does not copy text and trip the server's text-coverage check.

By default `generate` produces **distinct** concepts per variant (not N samples of one prompt). If you set `--daily` and the budget ÷ variants falls below ~$15/creative/day, it warns that some creatives may not clear the learning phase. You can also hand-place your own images or a video into the folders instead of generating them.

## 3. Ad copy

Ad copy is **campaign-level**, not per-image: Meta applies one headline × primary-text set across every creative, so you author it once. `generate` seeds `headlines` and `primaryTexts` arrays into **`campaign.json`** from your game's name and description — edit them there (up to 5 of each; headline ≤ 40 chars, primary text ≤ 125) before submitting.

The images themselves stay text-free, and the numbered per-creative sidecars (`square/0.json`, …) carry only prompt / seed / references — **not** copy. Putting copy in a sidecar does nothing (a legacy `square/0.json` is read once for back-compat, but `campaign.json` is the source of truth). `submit` rejects the campaign if the resolved copy is still a placeholder, too long, or has more than 5 options.
...
This uploads your assets and submits the campaign. The campaign is created **paused** — it is **not spending yet**. The RUN.world team reviews it, and once approved it's flighted (goes live).
...
| `auto-paused-performance` | Auto-paused by the performance guard for clear underperformance (unspent budget refunded). `stats`/`status` show the reason. |
...
- A campaign never spends until it's approved and flighted by the RUN.world team.
...
- Ad spend uses the provider-cost conversion of **1,000 credits per $1 USD**, before the ad-spend markup. Credit top-ups use a separate purchase rate: at the current default, **$1 USD adds 500 credits**. Use the ad-spend rate, not the top-up rate, when estimating campaign debits.
...
| Kind | Aspect Ratio | Size (px) | Format | Max File Size | Max Variants | Generated by default |
| --- | --- | --- | --- | --- | --- | --- |
| Square | 1:1 | 1080 × 1080 | PNG | 8 MB | 4 | yes |
| Vertical | 9:16 | 1242 × 2208 | PNG | 8 MB | 4 | no (`--kind vertical`) |
| Landscape | 1.91:1 | 1200 × 628 | PNG | 8 MB | 4 | no (`--kind landscape`) |

Only the `square` creative is required at submit; `vertical` and `landscape` are optional.

Generated `square` and `landscape` creatives are checked for baked-in text after generation. If detected text covers **4% or more** of a generated `square`, generation fails and you must regenerate it; the same threshold is advisory for generated `landscape` art and does not reject it. Keep logos and wordmarks small when using `composite` so a square stays below the limit.
...
`promo` renders a platform-sized promo image you can attach to a post. TikTok and Instagram are search-only, so a strong image with the game name on-screen matters most there.

```bash
# Size a promo image for a platform (required), described by game name…
rundot socials promo --platform instagram --name "My Game"

# …or with a full prompt (takes precedence over --name), to a custom path
rundot socials promo --platform x --prompt "neon arcade key art, bold title" --out ./key-art.png
```

| Option | Purpose |
| --- | --- |
| `--platform <p>` | Platform to size for (required): `discord`, `x`, `reddit`, `tiktok`, `instagram`. |
| `--prompt <text>` | Full image prompt. Takes precedence over `--name`. |
| `--name <text>` | Game name — builds a default promo prompt when `--prompt` is omitted. |
| `--out <path>` | Output file path (defaults to `./promo-<platform>.png`). |
| `--json` | Machine-readable output. |
