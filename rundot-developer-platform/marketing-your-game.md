...
Each `square` creative carries headline and primary-text options in its `.json` sidecar (Meta supplies them separately from the image and optimizes delivery across them). Edit the `headlines` and `primaryTexts` arrays (up to 5 each) before submitting — creatives themselves stay text-free.
...
# edit prompts in campaign.json + fill headlines/primaryTexts in the square sidecars, then:
rundot marketing generate --name spring-push
rundot marketing submit   --name spring-push
```
