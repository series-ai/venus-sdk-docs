...
# (--note is optional: it tells the RUN team why)
rundot marketing cancel --name spring-push --note "CPI too high, relaunching with new art"
```
...
`promo` renders platform-sized promo art grounded in the game's description,
thumbnail, and useful images from its deployed build. Every image returned by the
provider is saved; multiple results receive numbered filename suffixes. TikTok and
Instagram are search-only, so add the game name when composing the final post.
