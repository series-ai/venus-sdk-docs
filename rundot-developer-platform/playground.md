...
Install `firebase` and `@revenuecat/purchases-js` as `devDependencies`. They are used only by the local Playground sandbox. The playground plugin injects its host only during `vite dev`, so these packages are not part of the deployed RUN.game build.
