...
To test player-specific flows, open the local Vite URL in multiple browser tabs. Open them one at a time — let each tab finish loading before opening the next, so the coordinator assigns each a distinct slot.
