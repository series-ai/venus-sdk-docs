...
- For host fullscreen, use the [System API](api/SYSTEM.md#fullscreen-and-pointer-input-asynchronous). Do not call `Element.requestFullscreen()` or `Element.requestPointerLock()`.

RUN.world draws its own UI in the same viewport as your game. This UI includes
the toolbar, sheets, ads, and dialogs. The host enters fullscreen so this UI
stays visible. The game iframe cannot enter fullscreen or pointer lock directly.
`RundotGameAPI.system.getSafeArea()` reports the usable area with host UI and
device notches.
...
- Size your game to the [Safe Area](api/SAFE_AREA.md). Use the [System API](api/SYSTEM.md#fullscreen-and-pointer-input-asynchronous) when the player asks for fullscreen.
