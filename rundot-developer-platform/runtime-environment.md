...
- For filling the screen, lay out to the [Safe Area](api/SAFE_AREA.md) instead of calling `Element.requestFullscreen()` on your canvas or a container. Browser fullscreen is not a supported way to size your game.

RUN.world draws its own UI in the same viewport as your game: the toolbar, share and compose sheets, the ad player, and modal dialogs. An element in browser fullscreen paints over all of it, so a host dialog that opens while your game is fullscreen ends up behind the game, where the player can neither see nor dismiss it. `RundotGameAPI.system.getSafeArea()` reports the usable area with host chrome and device notches already accounted for, and it works the same way on web and mobile.
...
- Size your game to the [Safe Area](api/SAFE_AREA.md) rather than requesting browser fullscreen, which paints over host UI.
