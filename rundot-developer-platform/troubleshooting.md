...
Browser fullscreen isn't a supported way to size your game. RUN.world draws its toolbar, share sheets, ad player, and modal dialogs in the same viewport, and a fullscreen element paints over them, leaving the player with host dialogs they can't see or dismiss. Lay out to the safe area instead: `RundotGameAPI.system.getSafeArea()` reports the usable bounds with host chrome and device notches already accounted for. See [Safe Area](api/SAFE_AREA.md) and [Runtime Environment → Use the SDK instead](runtime-environment.md#use-the-sdk-instead).
