...
const EXCLUDED_FILES = new Set(['SUMMARY.md', 'AVATAR_3D.md'])
...
function generateIndexContent(files: string[]): string {
  ...
  return (
    [
      ...
      'IMPORTANT:To run/test this game locally, use `npm run dev` (Vite). SDK calls are mocked by default; they hit the real RUN.world playground backend only when rundotGamePlaygroundPlugin() is configured — see rundot-developer-platform/playground.md.',
      dirEntries.join('|'),
    ].join('|') + '\n'
  )
}
